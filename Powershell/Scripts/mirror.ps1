...
function mir-black {
    $jobs = @(
        ...
        @{ Mode = 'copy'; Source = 'X:\apps\comfy_ui_files\output'; Destination = 'X:\media\comfy_ui'; LogName = 'copy_ai_comfy_ui' }
...
function mir-ai-media {
    ...
    # ComfyUI: outputs -> media library
    Invoke-RoboLocal `
        -Mode 'copy' `
        -Source 'X:\apps\comfy_ui_files\output' `
